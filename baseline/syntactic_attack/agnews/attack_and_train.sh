...
device=cuda:0
data_path=$repo_path/data/agnews
batch=64
epoch=70
#for file in train test valid; do
#  if [ ! -d $repo_path/backdoor_attack/syntactic_attack/agnews ]; then
#    mkdir -p $repo_path/backdoor_attack/syntactic_attack/agnews
#  fi
#  cp $data_path/$file.tsv $repo_path/baseline/syntactic_attack/agnews
#  python $repo_path/baseline/syntactic_attack/generate_poison_file_from_attack.py --poison_file_path $data_path/$file.tsv \
#    --output_file_path $repo_path/backdoor_attack/syntactic_attack/agnews/ --file_usage $file --target_num 4
#done
for epsilon in 1; do
